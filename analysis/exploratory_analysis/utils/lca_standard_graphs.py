...
def build_comparison_table(dfs,
                           names,
                           fillna=None,
    ...
    comp = comp.reindex(reindex, level=0)
...
def plot_grouped_stackedbars(df, ix_categories, ix_entities_compared, norm='max', err_pos=None, err_neg=None,
    ...
    fig, (ax, ax2) = plt.subplots(1, 2, gridspec_kw = {'width_ratios':[100, 2]},
    ...
    for i, ent in enumerate(all_entities):
        ...
        sub.plot.barh(ax=ax, stacked=True, position=i, width=width, zorder=-1,
                      color=sns.color_palette(palettes[0]), edgecolor=edgecolor, label='_nolegend_')
